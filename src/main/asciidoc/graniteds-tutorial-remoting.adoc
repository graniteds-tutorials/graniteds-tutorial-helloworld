...
----
mvn clean install asciidoctor:process-asciidoc -Dserver={server} -Dclient={client}
----
...
ifeval::["{client}" == "flex"]
To run the Flex client, browse http://localhost:8080/helloworld/helloworld.swf[^]
endif::[]

ifeval::["{client}" == "air"]
Install and run the +.air+ package built in +target/helloworld.air+.
endif::[]
